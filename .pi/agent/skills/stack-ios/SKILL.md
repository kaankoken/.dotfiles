...
Load this file by path `~/.pi/agent/skills/stack-ios/SKILL.md` — not `skill://stack-ios` (not cold-listed).
...
1. Prefer path load if catalogued; else `read axiom-swiftui` (or related `axiom-*`) only if present in the session catalog (e.g. after enabling pack skills via Pi settings/packages or `/stack-ios`).
...
User installs missing underlying Axiom/iOS skills. This pack only routes; it does not vendor marketplace bodies.
