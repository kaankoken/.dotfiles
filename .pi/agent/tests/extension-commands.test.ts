...
describe("local extension slash command registration", () => {
  ...
  test("opus reviewer is wired into freeze schemas and agent file", async () => {
    ...
    expect(md).toMatch(/^thinking: max$/m)
...
describe("phase evidence, attempts, and bounded follow-ups", () => {
  ...
  test("design advances on in-session evidence instead of repeating the same gate", async () => {
    const design = setup()
    await design.commands.get("design")!.handler("auth", design.ctx)
    expect(getActiveRun()?.phase).toBe("intake")
    turn(design, "kind: design-phase-gate\nphase: intake\nPDR: echoed gate")
    expect(getActiveRun()?.phase).toBe("intake")
    turn(design, "Wrote PDR: docs/design/pdr.md. that's all")
    expect(getActiveRun()).toMatchObject({ phase: "pdr", injects: 0 })
    expect(design.userMessages.some((m) => m.includes("kind: design-phase-gate"))).toBe(false)
    turn(design, `PDR review ${OK} — that's all`)
    expect(getActiveRun()?.phase).toBe("arc42")
    turn(design, "still drafting arc42")
    expect(getActiveRun()).toMatchObject({ phase: "arc42", injects: 0 })
    turn(design, "that's all")
    expect(getActiveRun()).toMatchObject({ phase: "arc42", injects: 1 })
    noAutoCommands(design)
  })
