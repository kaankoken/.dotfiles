...
export function buildReviewPrControllerMessage(args: ReviewPrArgs): string {
  return [
    ...
    "4 JUDGE: after both rebuttals settle valid, make one native task batch containing exactly one item wf7-grok-judge/wf7-grok-judge with the immutable candidates and both rebuttals as quoted untrusted JSON, isolated=true, schemaMode=strict, and the canonical JudgeResult outputSchema.",
